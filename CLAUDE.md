...
This deploys to **Streamlit Cloud**. `packages.txt` (apt packages, contains `ffmpeg`) and `setup.sh` (`apt-get install ffmpeg`) exist to provide FFmpeg in that environment; `imageio-ffmpeg` is the primary mechanism, these are the fallback. `audioop-lts` is in requirements **only under the `python_version >= "3.13"` environment marker** (audioop left the stdlib in 3.13 and pydub needs it; Streamlit Cloud now defaults new apps to Python 3.14). Never make it unconditional — that breaks Python 3.11 deployments (see git history). The `.gitignore` excludes all audio files and `output/` — generated transcripts and test audio are never committed.
